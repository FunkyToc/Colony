...
# player interact
execute as @a[scores={Insect_DmgDealt=1..}] at @s if entity @e[tag=vorager,nbt={HurtTime:10s},distance=..6] run function colony:vorager/action/capture
...
# food path
execute as @e[type=minecraft:bat,tag=vorager,scores={Insect_Timer=..0}] at @s positioned ~ ~0.2 ~ run function colony:vorager/action/find_food
...
execute as @e[type=minecraft:bat,tag=vorager,scores={Insect_Timer=..-1}] if score @s Insect_Timer <= vora_death Insect_Timer run function colony:vorager/tool/kill
